...
http://localhost:5173
🔑 Authentication Flow
User logs in through the React frontend.
Credentials are sent to the Spring Boot authentication API.
...
Role-based authorization determines whether the user can access the requested operation.
Login
  ↓
Authentication
  ↓
...
Instead of immediately removing a record from the database, the system can mark it inactive.

This helps preserve existing academic data while preventing inactive records from appearing in normal active-record queries.
...
https://github.com/preetikotabagi
